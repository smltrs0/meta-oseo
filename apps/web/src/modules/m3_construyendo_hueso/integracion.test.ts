// Prueba de integración del módulo 3 ("Construyendo hueso"): monta el anfitrión REAL
// (ModuloView) con el content.json real del módulo y los seis componentes de actividad reales
// (sin sustituir ninguno), y recorre las ocho secciones como lo haría un estudiante:
...
const svgsPublicos = import.meta.glob('/public/images/m3/*.svg', {
  query: '?raw',
  import: 'default',
  eager: true,
...
const N = 3;
/** Módulos 1 a 3 completados: sin ellos el módulo 3 está bloqueado por secuencia. */
const COMPLETADOS_PREVIOS = [1, 2];
...
if (!carga.ok) throw new Error(`No se pudo cargar el módulo 3: ${carga.mensaje}`);
...
beforeAll(async () => {
  ...
  api = simularApi({
    ...
    responder: (llamada) => {
      const svg = /^\/images\/m3\/([a-z0-9_]+\.svg)$/.exec(llamada.ruta);
      if (svg) {
        const texto = svgsPublicos[`/public/images/m3/${svg[1]}`];
...
describe('módulo 3 integrado: cabecera y bloqueo inicial', () => {
  ...
  it('arranca en la sección 3.1 y las demás están bloqueadas hasta terminarla', async () => {
    expect(seccionActual().attributes('data-seccion')).toBe('m3_1_dos_rutas');
    await irA('m3_3_condensacion_laminar');
...
describe('módulo 3 integrado: las ocho secciones', () => {
...
describe('módulo 3 integrado: cierre y API', () => {
  it('se probaron los enlaces del glosario de todas las secciones', () => {
    // Un enlace por término del glosario que se enlazó en un texto o aviso de sección (41 de 46:
    // Smad, Noggin, Cilio primario y Frizzled solo están en tablas y actividades; integrina en un objetivo).
    const esperados = modulo.secciones
      .flatMap((s) => s.bloques)
      .flatMap((b) => (b.tipo === 'texto' || b.tipo === 'callout' ? [b.markdown] : []))
      .join(' ')
      .match(/\]\(glosario:/g)?.length;
    expect(esperados).toBe(41);
    expect(enlacesProbados).toBe(esperados);
  });
  ...
  it('(d) el módulo se marca completado con un PUT {completado: true} solo después de todos los POST', async () => {
    await esperar(() =>
      expect(
        api
          .filtrar('PUT', '/progress/3')
          .some((l) => (l.cuerpo as { completado?: boolean }).completado === true),
      ).toBe(true),
    );
    const puts = api.filtrar('PUT', '/progress/3');
...
describe('módulo 3 integrado: zonas táctiles de los dibujos reales', () => {
...
/* -------------------------------------------------------------------------------------------
 * Actividades opcionales: no bloquean la secuencia (3.5 y 3.6 tienen una cada una)
 * ----------------------------------------------------------------------------------------- */

describe('módulo 3 integrado: las actividades opcionales no bloquean la siguiente sección', () => {
  it('con solo las obligatorias de 3.1 a 3.5 hechas, la sección 3.6 está abierta; y sin las de 3.5, no', async () => {
    // Se retira el módulo montado por el recorrido anterior y se monta uno nuevo con el progreso
    // que devolvería el servidor: obligatorias hechas, opcionales sin tocar.
    wrapper.unmount();
    document.body.innerHTML = '';

    const ubicadas = listarActividades(modulo);
    const hechas = (hasta: number, salvo: string[] = []) =>
      ubicadas
        .filter(
          (u) =>
            modulo.secciones.indexOf(u.seccion) < hasta &&
            u.actividad.obligatoria &&
            !salvo.includes(u.actividad.id),
        )
        .map((u) =>
          fila(u.actividad.id, N, {
            tipo: u.actividad.tipo,
            mejor_puntaje: u.actividad.puntaje_max,
            completada: true,
          }),
        );

    async function montar(filas: ReturnType<typeof hechas>): Promise<VueWrapper> {
      localStorage.clear();
      const pinia = createPinia();
      setActivePinia(pinia);
      const auth = useAuthStore();
      auth.token = 'jwt-de-prueba';
      auth.establecerUsuario(usuarioDePrueba());
      api = simularApi({ completados: COMPLETADOS_PREVIOS, filas });
      const r = createRouter({
        history: createMemoryHistory(),
        routes: [
          { path: '/', name: 'inicio', component: { render: () => null } },
          { path: '/modulo/:n', name: 'modulo', component: { render: () => null } },
          { path: '/certificado', component: { render: () => null } },
        ],
      });
      await r.push({ name: 'modulo', params: { n: N }, query: { s: 'm3_6_wnt_esclerostina_pth' } });
      await r.isReady();
      router = r;
      const w = mount(ModuloView, {
        props: { n: N },
        attachTo: document.body,
        global: { plugins: [pinia, r] },
      }) as unknown as VueWrapper;
      await esperar(() => expect(w.find('[data-testid="cabecera-modulo"]').exists()).toBe(true));
      return w;
    }

    // 1) Obligatorias de 3.1 a 3.5 hechas; la opcional m3_multicapa_sensores (3.5) sin hacer.
    const obligatoriasHasta35 = hechas(5);
    expect(obligatoriasHasta35.some((f) => f.activity_id === 'm3_multicapa_sensores')).toBe(false);
    wrapper = await montar(obligatoriasHasta35);
    await esperar(() =>
      expect(wrapper.find('[data-testid="seccion-bloqueada"]').exists()).toBe(false),
    );
    expect(seccionActual().attributes('data-seccion')).toBe('m3_6_wnt_esclerostina_pth');
    expect(wrapper.get('[data-testid="avance-obligatorias"]').text()).toContain(
      `${obligatoriasHasta35.length} de 19`,
    );
    wrapper.unmount();
    document.body.innerHTML = '';

    // 2) Falta una obligatoria de 3.5: la sección 3.6 sigue bloqueada.
    wrapper = await montar(hechas(5, ['m3_quiz_cascada']));
    await esperar(() =>
      expect(wrapper.find('[data-testid="seccion-bloqueada"]').exists()).toBe(true),
    );
    expect(wrapper.find('[data-testid="seccion"]').exists()).toBe(false);
