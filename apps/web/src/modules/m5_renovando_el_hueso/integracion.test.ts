// Prueba de integración del módulo 5 ("Renovando el hueso"): monta el anfitrión REAL
...
const svgsPublicos = import.meta.glob('/public/images/m5/*.svg', {
  query: '?raw',
  import: 'default',
  eager: true,
...
const N = 5;
/** Módulos 1 a 4 completados: sin ellos el módulo 5 está bloqueado por secuencia. */
const COMPLETADOS_PREVIOS = [1, 2, 3, 4];
...
if (!carga.ok) throw new Error(`No se pudo cargar el módulo 5: ${carga.mensaje}`);
...
beforeAll(async () => {
  ...
  api = simularApi({
    ...
    responder: (llamada) => {
      const svg = /^\/images\/m5\/([a-z0-9_]+\.svg)$/.exec(llamada.ruta);
      if (svg) {
        const texto = svgsPublicos[`/public/images/m5/${svg[1]}`];
...
describe('módulo 5 integrado: cabecera y bloqueo inicial', () => {
  ...
  it('arranca en la sección 5.1 y las demás están bloqueadas hasta terminarla', async () => {
    expect(seccionActual().attributes('data-seccion')).toBe('m5_1_bmu_ciclo');
    await irA('m5_3_acoplamiento_esclerostina');
...
describe('módulo 5 integrado: las ocho secciones', () => {
  for (const [indice, seccion] of modulo.secciones.entries()) {
    ...
    it(`${seccion.id}: monta, se completa y abre ${siguiente?.id ?? 'el cierre del módulo'}`, async () => {
      ...
      // Los bloques de posgrado no se ven con el nivel por defecto (pregrado).
...
describe('módulo 5 integrado: cierre y API', () => {
  it('se probaron los enlaces del glosario de todas las secciones', () => {
    expect(enlacesProbados).toBeGreaterThanOrEqual(39);
  });
  ...
  it('(d) el módulo se marca completado con un PUT {completado: true} solo después de todos los POST', async () => {
    await esperar(() =>
      expect(
        api
          .filtrar('PUT', '/progress/5')
          .some((l) => (l.cuerpo as { completado?: boolean }).completado === true),
      ).toBe(true),
    );
    const puts = api.filtrar('PUT', '/progress/5');
...
/* -------------------------------------------------------------------------------------------
 * Nivel de posgrado: los bloques «Para profundizar» solo se ven con ese nivel
 * ----------------------------------------------------------------------------------------- */

describe('módulo 5 integrado: nivel de posgrado', () => {
  it('con nivel de posgrado aparecen los bloques de profundización de cada sección', async () => {
    const contexto = useContextoStore();
    try {
      let conPosgrado = 0;
      for (const seccion of modulo.secciones) {
        const posgrado = seccion.bloques.filter((b) => !visibleParaNivel(b, 'pregrado'));
        if (posgrado.length === 0) continue;
        conPosgrado++;
        const avisosPre = seccion.bloques.filter(
          (b) => visibleParaNivel(b, 'pregrado') && b.tipo === 'callout',
        ).length;
        const avisosTodos = seccion.bloques.filter((b) => b.tipo === 'callout').length;
        const textosPre = seccion.bloques.filter(
          (b) => visibleParaNivel(b, 'pregrado') && b.tipo === 'texto',
        ).length;
        const textosTodos = seccion.bloques.filter((b) => b.tipo === 'texto').length;

        contexto.setNivel('pregrado');
        await irA(seccion.id);
        expect(wrapper.findAll('[data-testid="bloque-callout"]'), seccion.id).toHaveLength(
          avisosPre,
        );
        expect(wrapper.findAll('[data-testid="bloque-texto"]'), seccion.id).toHaveLength(textosPre);
        ...
        expect(wrapper.findAll('[data-testid="bloque-texto"]'), seccion.id).toHaveLength(
          textosTodos,
        );
      }
      expect(conPosgrado).toBe(3);
    } finally {
      contexto.setNivel('pregrado');
    }
  }, 60_000);
...
describe('módulo 5 integrado: zonas táctiles de los dibujos reales', () => {
